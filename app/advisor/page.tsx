...
function AdvisorContent() {
  ...
  const isHuman = searchParams.get("human") === "true";

  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: isHuman
        ? "Hey Maya! Priya here 👋 I just got pulled into your thread — I've already seen your Due North verdict and your account info. What can I help you figure out?"
        : "Hey Maya! 👋 I'm your Due North AI. Saw you just ran that TikTok through the analyzer. Got any questions about it, or anything else investing-related?",
    },
  ]);
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [mode, setMode] = useState<"ai" | "human">(isHuman ? "human" : "ai");
    ...
    if (mode === "human") {
      ...
      setTimeout(() => {
        setMessages([
          ...
          {
            role: "assistant",
            content: "Great question — let me pull that up for you. Based on your situation, I'd suggest we look at a couple of options. Want me to walk you through them?",
          },
        ]);
        setLoading(false);
      }, 1800);
    ...
    <div className="h-full flex flex-col" style={{ background: "#f7f5f2" }}>
      <div className="px-5 pt-3 pb-3 bg-white border-b border-stone-200 flex items-center gap-3 flex-shrink-0">
        ...
        <div className="relative">
          <div className={`w-9 h-9 rounded-full flex items-center justify-center ${mode === "human" ? "bg-scotia-red" : "bg-stone-900"}`}>
        ...
        <div className="flex-1">
          <div className="text-sm font-semibold">
            {mode === "ai" ? "Due North AI" : "Priya Sharma"}
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 scrollbar-hide">
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex gap-2 fade-up ${m.role === "user" ? "justify-end" : ""}`}>
            {m.role === "assistant" && (
              <div className={`w-7 h-7 rounded-full flex items-center justify-center flex-shrink-0 ${mode === "human" ? "bg-scotia-red" : "bg-stone-900"}`}>
      ...
      <div className="bg-white border-t border-stone-200 p-3 flex items-center gap-2 flex-shrink-0">
        ...
        <input
          ...
          placeholder={mode === "ai" ? "Ask Due North AI..." : "Message Priya..."}
